...
Face.prototype.link = function(face, v0, v1) {
  ...
  if (face instanceof Face) {
    ...
    if (twin === null) {
      throw "ERROR: twin is null";
    }
    edge = this.getEdge(v0, v1);
    twin.twin = edge;
    edge.twin = twin;
  } else {
    var e = face;
    edge = this.getEdge(e.orig, e.dest);
    e.twin = edge;
    edge.twin = e;
  }
}
...
var ConvexHull = {
  ...
  prep: function() {
    ...
    if (this.points.length <= 3) {
      throw "ERROR: Less than 4 points";
    }
    ...
    var v;
    for (i = this.current; i < this.points.length; i++) {
      v = this.points[i];
      if (f0.conflict(v)) {
